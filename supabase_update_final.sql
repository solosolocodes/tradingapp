...
-- First checking how 'options' column is defined
DO $$
DECLARE
  column_type TEXT;
BEGIN
  -- Get the data type of the options column
  ...
  -- Insert age question with null options
  ...
  -- Insert gender question with appropriate options format based on column type
  IF column_type = 'jsonb' THEN
    -- If options is JSONB, insert as JSON array
    INSERT INTO public.experiment_survey_questions (experiment_id, question, type, options, order_index, is_demographic)
    SELECT 
      e.id, 
      'What is your gender?', 
  ...
  ELSIF column_type = 'ARRAY' THEN
    -- If options is an array, insert as text array
    INSERT INTO public.experiment_survey_questions (experiment_id, question, type, options, order_index, is_demographic)
    SELECT 
      e.id, 
      'What is your gender?', 
    ...
    AND e.id IS NOT NULL;
  ELSE
    -- If neither, insert options as NULL and deal with it in the application
    INSERT INTO public.experiment_survey_questions (experiment_id, question, type, options, order_index, is_demographic)
    SELECT 
      e.id, 
      'What is your gender?', 
