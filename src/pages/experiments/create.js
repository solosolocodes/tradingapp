...
export default function CreateExperiment() {
  ...
  const handleToggleControlGroup = (groupId) => {
    setSelectedGroups(selectedGroups.map(g => 
      g.id === groupId ? {...g, is_control: !g.is_control} : g
    ));
  };
        ...
        if (section.type === 'info') {
          ...
          await supabase
            ...
            .insert({
              ...
              round_duration: templateToUse.round_duration || 10, // Default to 10 seconds per round
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Redirect to experiment edit page to see and edit experiment content
                            ...
                            <div style={{ display: 'flex', gap: 'var(--spacing-sm)' }}>
                              <button 
                                ...
                                style={{ 
                                  padding: '2px 6px', 
                                  fontSize: '0.8rem',
                                  backgroundColor: group.is_control ? 'var(--color-gray)' : 'var(--color-warning)'
                                }}
                                onClick={() => handleToggleControlGroup(group.id)}
                              >
                                {group.is_control ? 'Remove Control' : 'Set as Control'}
                              </button>
                              <button 
                                type="button" 
                                className="danger"
