  ...
  const fetchSections = async () => {
    try {
      ...
      const formattedScenarios = (scenarios || []).map(scenario => ({
        ...
        round_duration: scenario.round_duration || 10, // Default to 10 seconds if not set
  ...
  const createDefaultSections = async (experimentId) => {
    try {
      ...
      const { data: scenarioTemplates, error: scenarioError } = await supabase
        .from('scenario_templates')
        .select('id, title, description, round_duration, option_template')
        ...
        if (section.type === SECTION_TYPES.INFO) {
          ...
          await supabase
            ...
            .insert({
              ...
              round_duration: templateToUse.round_duration || 10,
              order_index: section.order_index
            });
        }
      ...
      switch (type) {
        ...
        case SECTION_TYPES.SCENARIO:
          ...
          const { data: templateData, error: templateError } = await supabase
            .from('scenario_templates')
            .select('title, description, round_duration, option_template')
          ...
          response = await supabase
            ...
            .insert({
              ...
              round_duration: templateData.round_duration || 10, // Default to 10s if not set
          ...
          newSection = {
            ...
            round_duration: response.data.round_duration || 10, // Default to 10s
                      ...
                      {section.type === SECTION_TYPES.SCENARIO && (
                        <div>
                              ...
                              onChange={async (e) => {
                                ...
                                try {
                                  ...
                                  const { data: template, error: templateError } = await supabase
                                    .from('scenario_templates')
                                    .select('title, description, round_duration')
                                  ...
                                  const { error: updateError } = await supabase
                                    ...
                                    .update({
                                      ...
                                      round_duration: template.round_duration || 10 // Default to 10s if not set
                                    })
                                    .eq('id', section.originalId);
                          ...
                          <div style={{ 
                            ...
                            <div style={{ marginBottom: '8px' }}>
                              <div style={{ fontSize: '0.8rem', fontWeight: 'bold', marginBottom: '2px' }}>Duration per Round:</div>
                              <div style={{ fontSize: '0.85rem', padding: '6px', backgroundColor: 'white', borderRadius: '4px', border: '1px solid var(--color-gray)' }}>
                                {section.round_duration || 10} seconds
                              </div>
                            </div>
                          </div>
