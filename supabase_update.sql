...
-- Step 8: Create default demographic survey questions if none exist
...
INSERT INTO public.experiment_survey_questions (experiment_id, question, type, options, order_index, is_demographic)
SELECT e.id, 'What is your gender?', 'multiple_choice', ARRAY['Male', 'Female', 'Non-binary', 'Prefer not to say'], 0, TRUE
FROM public.experiments e
WHERE EXISTS (
  SELECT 1 FROM public.experiment_survey_questions sq
  WHERE sq.experiment_id = e.id AND sq.is_demographic = TRUE
...
-- Step 9: Update RLS policies to ensure proper access to experiment_scenarios
...
-- Step 10: Fix any experiment sections that have null fields
